...
func (q *qiniu) List(prefix, marker, delimiter string, limit int64) ([]Object, error) {
	...
	for i := 0; i < n; i++ {
		...
		objs[i] = &obj{entry.Key, entry.Fsize, time.Unix(mtime, 0), strings.HasSuffix(entry.Key, "/"), map[string]any{}}
	}
	if delimiter != "" {
		for _, p := range prefixes {
			objs = append(objs, &obj{p, 0, time.Unix(0, 0), true, map[string]any{}})
		}
